...
def icp_scan_matching_1d(global_map, local_scan, max_iterations=100, tolerance=1e-7, num_initial_guesses=5):
    def compute_error(translation, global_map, local_scan):
        """ Compute the cost based on distance between the shifted local_scan and the global_map """
        # Translate only along the x-axis (shift the position of the local_scan without altering its values)
        transformed_scan_x = np.arange(len(local_scan)) + translation  # Shifted x positions of local scan
        
        # Clip transformed scan x positions so they stay within global map bounds
        transformed_scan_x = np.clip(transformed_scan_x, 0, len(global_map) - 1)
        
        # Compute absolute distance between the corresponding points
    ...
    for p in range(num_initial_guesses):
        # Generate random initial guess for translation along the global map (x-axis only)
        initial_guess = np.random.uniform(0, len(global_map) - len(local_scan))  # Random x translation

        # Plot the initial guess (local scan translated by initial_guess)
        plt.plot(np.arange(len(local_scan)) + initial_guess, local_scan, c='red', label='Local Scan (initial)', alpha=0.5)

        # Run optimization with the current random initial guess
        result = minimize(
            compute_error, 
            initial_guess, 
        ...
        if result.fun < best_cost:
            ...
            best_translation = result.x

    # Compute best transformed scan using the best translation
    best_transformed_scan_x = np.arange(len(local_scan)) + best_translation
    best_transformed_scan_x = np.clip(best_transformed_scan_x, 0, len(global_map) - 1)
    best_transformed_scan = global_map[best_transformed_scan_x.astype(int)]

    # Plot the final transformed local scan
...
results = icp_scan_matching_1d(resampledkappa, kappalocal, max_iterations=100, tolerance=1e-7, num_initial_guesses=100)
# # print(results)
