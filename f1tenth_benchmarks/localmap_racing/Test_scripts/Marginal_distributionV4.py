...
def project_distribution_on_axes(mean, cov_matrix, angle):
    # Create rotation matrix
    rotation_matrix = np.array([[np.cos(angle), -np.sin(angle)],
                                 [np.sin(angle),  np.cos(angle)]])
    
    # Calculate new mean
    new_mean = rotation_matrix @ np.array(mean[:2])
...
new_mean, new_cov = project_distribution_on_axes(mean, cov_matrix, random_angle)
...
ax[0].plot([0, 6 * np.cos(random_angle)], [0, 6 * np.sin(random_angle)], color='orange', lw=2, label='New Axis 1')
ax[0].plot([0, 6 * np.cos(random_angle + np.pi/2)], [0, 6 * np.sin(random_angle + np.pi/2)], color='purple', lw=2, label='New Axis 2')

# Plotting the new covariance ellipse
plot_covariance_ellipse(ax[0], new_mean, new_cov, edgecolor='orange', fc='none', lw=2)
